//
//  VulkanBlitCommandEncoder.swift
//  VkRenderer
//
...
//
  ...
  class VulkanBlitCommandEncoder: VulkanCommandEncoder {
    ...
    public init(device: VulkanDevice, commandBuffer: VulkanCommandBuffer, resourceMap: VulkanPersistentResourceRegistry) {
      self.device = device
    ...
    func executePass(_ pass: RenderPassRecord, resourceCommands: [CompactedResourceCommand<VulkanCompactedResourceCommandType>]) {
      ...
      for (i, command) in zip(pass.commandRange!, pass.commands) {
        ...
        executeCommand(command, commandIndex: i)
    ...
    func executeCommand(_ command: RenderGraphCommand, commandIndex: Int) {
      ...
      case let .copyBufferToTexture(args):
      ...
      case let .copyBufferToBuffer(args):
        let source = resourceMap[args.pointee.sourceBuffer]!
        ...
        vkCmdCopyBuffer(commandBufferResources.commandBuffer, source.buffer.vkBuffer, destination.buffer.vkBuffer, 1, &region)

      case let .copyTextureToBuffer(args):
        fatalError("Unimplemented.")

      case let .copyTextureToTexture(args):
        ...
        regions.withUnsafeBufferPointer { regions in
          ...
          vkCmdCopyImage(
            ...
            source.vkImage, source.layout(commandIndex: commandIndex, subresourceRange: ActiveResourceRange.fullResource),
            destination.vkImage, destination.layout(commandIndex: commandIndex, subresourceRange: ActiveResourceRange.fullResource),
            UInt32(regions.count), regions.baseAddress
          )
        }

      case let .fillBuffer(args):
        ...
        vkCmdFillBuffer(commandBufferResources.commandBuffer, buffer.buffer.vkBuffer, VkDeviceSize(args.pointee.range.lowerBound) + VkDeviceSize(buffer.offset), VkDeviceSize(args.pointee.range.count), intValue)

      case let .generateMipmaps(texture):
        fatalError("Mipmap generation should be handled by a series of blits at the RenderGraph level")

      case let .blitTextureToTexture(args):
        ...
        vkCmdBlitImage(commandBufferResources.commandBuffer, sourceImage.vkImage, sourceLayout, destImage.vkImage, destLayout, 1, &region, VkFilter(args.pointee.filter))

      case let .synchroniseTexture(textureHandle):
        fatalError("GPU to CPU synchronisation of managed resources is unimplemented on Vulkan.")

      case let .synchroniseTextureSlice(args):
        fatalError("GPU to CPU synchronisation of managed resources is unimplemented on Vulkan.")

      case let .synchroniseBuffer(buffer):
        fatalError("GPU to CPU synchronisation of managed resources is unimplemented on Vulkan.")
